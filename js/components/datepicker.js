...
!((a) => {
	...
	k.prototype = {
		constructor: k,
		_resolveViewName: (b) => (
			a.each(r.viewModes, (c, d) => {
				if (b === c || -1 !== a.inArray(b, d.names)) return (b = c), !1;
			}),
			b
		),
		_resolveDaysOfWeek: (b) => (
			Array.isArray(b) || (b = b.split(/[,\s]*/)), a.map(b, Number)
		),
		_check_template: (c) => {
			try {
				if (c === b || "" === c) return !1;
		...
		_process_options: function (b) {
			...
			e.startDate !== -1 / 0 &&
				...
				(e.datesDisabled = a.map(e.datesDisabled, (a) =>
					r.parseDate(a, g, e.language, e.assumeNearbyYear),
				));
			var h = String(e.orientation).toLowerCase().split(/\s+/g),
				i = e.orientation.toLowerCase();
			if (
				((h = a.grep(h, (a) => /^auto|left|right|top|bottom$/.test(a))),
				...
				if (1 === h.length)
					...
					(i = a.grep(h, (a) => /^left|right$/.test(a))),
						(e.orientation.x = i[0] || "auto"),
						(i = a.grep(h, (a) => /^top|bottom$/.test(a))),
		...
		_local_to_utc: (a) =>
			a && new Date(a.getTime() - 6e4 * a.getTimezoneOffset()),
		_zero_time: (a) =>
			a && new Date(a.getFullYear(), a.getMonth(), a.getDate()),
		_zero_utc_time: (a) =>
			a && c(a.getUTCFullYear(), a.getUTCMonth(), a.getUTCDate()),
		getDates: function () {
			return a.map(this.dates, this._utc_to_local);
		},
		getUTCDates: function () {
			return a.map(this.dates, (a) => new Date(a));
		},
		getDate: function () {
		...
		getFormattedDate: function (c) {
			...
			return a
				.map(this.dates, (a) => r.formatDate(a, c, d))
		...
		place: function () {
			...
			if (
				...
				"top" === p
					? (n -= c + Number.parseInt(this.picker.css("padding-top")))
					: (n += k),
				this.o.rtl)
			) {
				var q = e - (m + l);
		...
		setRange: function (b) {
			b && b.length
				? (this.range = a.map(b, (a) => a.valueOf()))
				: delete this.range,
				this.fill();
		},
		...
		_fill_yearsView: function (c, d, e, f, g, h, i) {
			for (
				var j,
					...
					s = a.map(this.dates, (a) => Math.floor(a.getUTCFullYear() / n) * n),
					t = p - n;
				t <= q + n;
				t += n
			)
		...
		moveMonth: function (a, b) {
			...
			if (((b = b > 0 ? 1 : -1), 1 === i))
				(d =
					-1 === b ? () => e.getUTCMonth() === h : () => e.getUTCMonth() !== c),
					(c = h + b),
					e.setUTCMonth(c),
					(c = (c + 12) % 12);
			else {
			...
			while (d()) e.setUTCDate(--f), e.setUTCMonth(c);
			return e;
		},
		...
		dateIsDisabled: function (b) {
			return (
				...
				a.grep(this.o.datesDisabled, (a) => e(b, a)).length > 0
			);
		},
	...
	var l = function (b, c) {
		a.data(b, "datepicker", this),
			(this.element = a(b)),
			(this.inputs = a.map(c.inputs, (a) => (a.jquery ? a[0] : a))),
			delete c.inputs,
			...
			(this.pickers = a.map(this.inputs, (b) => a.data(b, "datepicker"))),
			this.updateDates();
	};
	l.prototype = {
		updateDates: function () {
			(this.dates = a.map(this.pickers, (a) => a.getUTCDate())),
				this.updateRanges();
		},
		updateRanges: function () {
			var b = a.map(this.dates, (a) => a.valueOf());
			a.each(this.pickers, (a, c) => {
				c.setRange(b);
			});
		},
		clearDates: function () {
			a.each(this.pickers, (a, b) => {
				b.clearDates();
			});
		},
		...
		r = {
			...
			parseDate: function (c, e, f, g) {
				...
				var t,
					...
					x = {
						yyyy: (a, b) => a.setUTCFullYear(g ? h(b, g) : b),
						m: (a, b) => {
							if (isNaN(a)) return a;
							for (b -= 1; b < 0; ) b += 12;
				...
				if (
					(j.length !== y.length &&
						(y = a(y)
							.filter((b, c) => -1 !== a.inArray(c, w))
							.toArray()),
					j.length === y.length)
				) {
					var z;
					for (n = 0, z = y.length; n < z; n++) {
						if (((t = Number.parseInt(j[n], 10)), (l = y[n]), isNaN(t)))
							switch (l) {
								case "MM":
