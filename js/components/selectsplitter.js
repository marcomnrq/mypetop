	...
	function t(t) {
		return this.each(function () {
			var l = e(this),
				...
				o = "object" == typeof t && t;
			(s || "destroy" != t) &&
				(s || l.data("selectsplitter", (s = new r(this, o))),
				"string" == typeof t && s[t]());
		});
	}
	var r = function (e, t) {
	...
	(r.DEFAULTS = {
		...
		(r.prototype.init = function (t, l, s) {
			...
			for (var c in o.fullCategoryList)
				o.fullCategoryList.hasOwnProperty(c) &&
					(i = i + "<option>" + c + "</option>");
			o.$element.after(o.options.template),
				(o.$wrapper = o.$element.next(
					"div[data-selectsplitter-wrapper-selector]",
				)),
				(o.$firstSelect = e(
					"select[data-selectsplitter-firstselect-selector]",
					o.$wrapper,
				)),
				(o.$secondSelect = e(
					"select[data-selectsplitter-secondselect-selector]",
					o.$wrapper,
				));
			...
			(d = Math.min(d, 10)),
				...
				o.$element.find("option[selected=selected]").length &&
					((o.$selectedOption = o.$element.find("option[selected=selected]")),
					(o.currentParentCategory = o.$selectedOption
						.closest("optgroup")
						.attr("label")),
					(o.currentChildCategory = o.$selectedOption.attr("value")),
					o.$firstSelect
						.find("option:contains(" + o.currentParentCategory + ")")
						.attr("selected", "selected"),
					o.$firstSelect.trigger("change"));
		}),
		(r.prototype.updateParentCategory = function () {
			(this.currentParentCategory = this.$firstSelect.val()),
				this.$secondSelect.empty();
			var t = "";
			for (var r in this.fullCategoryList[this.currentParentCategory])
				this.fullCategoryList[this.currentParentCategory].hasOwnProperty(r) &&
					(t =
						t +
						'<option value="' +
						this.fullCategoryList[this.currentParentCategory][r].x +
						'">' +
						this.fullCategoryList[this.currentParentCategory][r].y +
						"</option>");
			this.$secondSelect.append(t),
				this.$selectedOption &&
					this.$secondSelect
						.find('option[value="' + this.$selectedOption.attr("value") + '"]')
						.attr("selected", "selected");
		}),
		(r.prototype.updateChildCategory = function (t) {
			(this.currentChildCategory = e(t.target).val()),
				this.$element.find("option[selected=selected]").removeAttr("selected"),
				this.$element
					.find('option[value="' + this.currentChildCategory + '"]')
					.attr("selected", "selected"),
				this.$element.trigger("change"),
				(this.$selectedOption = this.$element.find(
